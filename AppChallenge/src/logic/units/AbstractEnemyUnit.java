...
public abstract class AbstractEnemyUnit extends AbstractUnit implements EnemyUnit {
	...
	protected int[][] getRowColCountsOfTeamUnits(Board board) {
		...
		for(int i = 0; i < board.getRows(); i++) {
			for(int j = 0; j < board.getCols(); j++) {
				if(board.getUnitAtOrNull(i, j) instanceof PlayerUnit) {
					rowUnitCounts[i]++;
					colUnitCounts[j]++;
				}
			}
	...
	protected Collection<PlayerUnit> teamUnits8Adjacent(Board board, int row, int col) {
		...
		for(int[] adj : Board.ADJACENT_8) {
			...
			Unit unit;
			if(board.inBounds(nr, nc) && (unit = board.getUnitAtOrNull(nr, nc)) instanceof PlayerUnit)
